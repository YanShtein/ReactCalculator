...
class App extends React.Component {
  ...
  DisplayBoth(event) {
    if (this.state.formula.length > 26 || this.state.result.length > 26 || this.state.result === displayError) {
      ...
      if (hasOperator.test(event.target.value)) { //if endswithoperator 
      this.setState({
      ...
      if (startsWithZero.test(this.state.result) || hasOperator.test(this.state.result)) {
      this.setState({
      ...
      this.setState({
        formula: this.state.formula.concat(event.target.value), //display FORMULA as entered & display left->right
        result: this.state.result + event.target.value //when value clicked, display value one by one in RESULT
      });
    };
    };
    };
